...
class ApplicationController < ActionController::API
    def common_ancestor
        ...
        if invalid_parameter_values.any?
            render status: :bad_request,
                   json: { message: "One or more invalid parameter values provided. Please replace the following parameter values with valid node ids: #{invalid_parameter_values.join(', ')}" }
            return
        end
        ...
        render status: :ok,
               json: { root_id: result[:root_id], lowest_common_ancestor: result[:lowest_common_ancestor], depth: result[:depth] }
    ...
    def birds
        ...
        render status: :internal_server_error,
               json: { message: "An error occurred while attempting to determine the list of birds: #{error.message}" }
    end
    ...
    def generate_csv_for_seeding
        ...
        # Handle bad requests
        invalid_parameter_values = []
        invalid_parameter_values << params[:number_of_trees] unless number_of_trees > 0
        invalid_parameter_values << params[:number_of_nodes_per_tree] unless number_of_nodes_per_tree > 0
        invalid_parameter_values << params[:number_of_birds_per_tree] unless number_of_birds_per_tree > 0

        if invalid_parameter_values.any?
            render status: :bad_request,
                   json: { message: "One or more invalid parameter values provided. Please replace the following parameter values with positive integers: #{invalid_parameter_values.join(', ')}" }
            return
        end
        ...
        render status: :ok,
               json: { message: 'Successfully generated CSV files for seeding. You may now use /seed_data_from_csv?node_path=public/<node_filename>&bird_path=public/<bird_filename> to seed data. ' }
    rescue StandardError => error
        render status: :internal_server_error,
               json: { message: "An error occurred while attempting to generate CSVs for seeding: #{error.message}" }
    end
    ...
    def seed_data_from_csv
        ...
        if invalid_parameter_values.any?
            render status: :bad_request,
                   json: { message: "One or more invalid parameter values provided. Please replace the following parameter values with valid file paths: #{invalid_parameter_values.join(', ')}" }
            return
        end
        ...
        node_values = node_data.map { |row| "(#{row[:id]}, #{row[:parent_id] || 'NULL'}, #{current_timestamp}, #{current_timestamp})" }.join(', ')
        bird_values = bird_data.map { |row| "(#{row[:id]}, #{row[:node_id]}, #{current_timestamp}, #{current_timestamp})" }.join(', ')
